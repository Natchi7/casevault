...
export default class PermanencyPlanApplicableChildAssessment extends UtilityBaseElement {
    ...
    getIntialAdoptionPlanStage() {
        ...
        .then(result => {
            if (result) {
                ...
                applicableAssRec = this.checkNamespaceApplicable(res.applicableChildAssRec, false);
    ...
    handleSave(event) {
        ...
        if(!this.onValidate()) {
            updateApplicableChildAssessment({applicableChildAssessmentRecJSON : JSON.stringify(this.checkNamespaceApplicable(this.applicableChildAssessmentRec,true)), resubmissionsign : this.resubmissionSignature, caseworkerSign : this.caseWorkerSign}).then(result => {
               this.title = "Sucess!";
               this.type = "success";
