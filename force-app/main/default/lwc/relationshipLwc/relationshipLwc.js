...
{label: 'Is Caregiver', fieldName: 'Is_Caregiver__c', type:'boolean'},
...
export default class RelationshipLwc extends UtilityBaseElement {
 ...
 connectedCallback() {
    ...
    .then(result=> {
        ...
        if(this.personRecordList.length > 0) {
...
handleSave() {
    ...
    this.loading = true;
    saveRelationship({relationshipJSON:JSON.stringify(this.checkNamespaceApplicable(this.relationshipRecord,true))})
    .then(result => {
        this.loading = false;
        this.handleRelation(); 
        this.showModal = false;
