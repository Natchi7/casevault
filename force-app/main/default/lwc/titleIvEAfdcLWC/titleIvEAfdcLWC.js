        ...
        getIncomes({ titleIVEId: this.titleIvERec.Id}).then(result => {
                ...
                getPickvalue({titleiveId : this.titleIvERec.Id}).then(result => {
                    ...
                    for(let i=0; i<this.deprivationPicklist.parentPicklist.length;i++) {
                        this.deprivationPicklist.parentPicklist[i].label = this.deprivationPicklist.parentPicklist[i].Name;
                        this.deprivationPicklist.parentPicklist[i].value = this.deprivationPicklist.parentPicklist[i].Id;
                    }
    ...
    handleIconAction(event) {
        ...
        switch (actionValue) {
            ...
            case 'delete':
                ...
                if (foundelement) {
                    delIncomeSummary({ incomeSummaryRec: JSON.stringify(this.checkNamespaceApplicable(foundelement, true)) }).then(result => {
